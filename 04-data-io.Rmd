...
Note: by default, `read_csv()` will only give a warning if columns cannot be converted to the specified type. To enforce an error use `stop_for_problems()`:

```{r}
read_chickens <- function(csv) {
  chickens <- read_csv(
    file = csv,
    col_types = cols(
      chicken = col_character(),
  ...
  stop_for_problems(chickens)

  chickens
}

chickens <- read_chickens(chickens_csv)
```
...
-   `read_xlsx()`: Read tabular data from an xlsx file.
...
-   `range`: Specify a range to read from. This can be in the format `"B3:D87"` or `"R1C3:R27C12"`. Alternately, several helper functions can be used for more flexible ranges:
    ...
    -   `anchored()`: Specify the top-left cell (anchor) and the dimensions.
...
While most use-cases can be covered by the above packages, there are some that are not. For example, if you need to fill out an Excel template, {writexl} will not be suitable. In that case {openxlsx} can be used.

Before using {openxlsx}, you should first review alternatives. For example, it might be better to export data as a separate Excel workbook using {writexl} then use lookups in the template. Using {openxlsx} is very powerful, but it may get difficult to maintain of the spreadsheet it is working with changes.
